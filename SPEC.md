...
knowledge
 ...
 — the metadata, context, and curated insight that surrounds
...
no required tooling. If you can 

cat

 a file, you can read OKF; if you
...
Readable
 ...
 by humans without tooling.

Parseable

 by agents without bespoke SDKs.
...
Portable
 ...
 across tools, organizations, and time.
...
Define a universal format that 
...
exchange
 ...
 of knowledge across systems and organizations.

Standardize the small number of 
...
Knowledge Bundle
 ...
 — A self-contained, hierarchical collection of
...
Concept
 ...
 — A single unit of knowledge within a bundle. Represented
...
Concept ID
 ...
 — The path of the concept's file within the bundle,
...
Frontmatter
 ...
 — YAML metadata block delimited by 
...
Link
 ...
 — A standard markdown link from one concept to another, used
...
Citation
 ...
 — A link from a concept to an external source that
...
Directory listing. See §6.

log.md

Update history. See §7.
...
Tags themselves remain a first-class concept — see the 
...
the start of the file and a closing 
...
type
 ...
 — A short string identifying the kind of concept. Consumers
...
not
 ...
 registered centrally. Producers SHOULD pick
...
tags
 ...
 — A YAML list of short strings for cross-cutting categorization.
...
Extensions:
 ...
 Producers MAY include any additional keys. Consumers
...
External sources backing claims in the body. See §8.
...
, interpreted relative to the bundle root.
...
recommended
 ...
 form because it is stable when documents are
...
A link from concept A to concept B asserts a 
...
index.md
 ...
 file MAY appear in any directory, including the bundle

 

root. It enumerates the directory's contents to support 

progressive

disclosure

 — letting a human or agent see what is available before
...
frontmatter. Producers MAY generate 
...
log.md
 ...
 file MAY appear at any level of the hierarchy to record the
...
Date headings MUST use ISO 8601 

YYYY-MM-DD

 form. Log entries are

 

prose; the leading bold word (
...
, etc.) is a convention, not a requirement.
...
those sources SHOULD be listed under a 
...
references/
 ...
 subdirectory that mirrors external material as
...
.md
 ...
 file in the tree contains a parseable YAML
...
LLM "wiki" repositories
 ...
 that use markdown + frontmatter as
...
Personal knowledge tools
 ...
 like Obsidian and Notion, which use
...
"Metadata as code"
 ...
 approaches that store catalog metadata

 

alongside source code rather than in a separate registry.

OKF differs primarily in being 

specified

 — pinning down the small
...
This document specifies OKF version 

0.1

. Future revisions will be
...
minor
 ...
 version bump introduces backward-compatible additions
...
major
 ...
 version bump may make breaking changes (renaming required
...
only place frontmatter is permitted in an 
